...
GameStateManager.prototype = {
    ...
    get_summary_for_session: function(sessionOrIndex, event_data) {
        var summary_data = []
        var game_session = null;

        if (typeof sessionOrIndex === 'number') {
            if (sessionOrIndex >= this.game_sessions.length) {
                return null;
            }
            game_session = this.game_sessions[sessionOrIndex];
        } else {
            game_session = sessionOrIndex;
        }
        ...
        for(var i = 0; i < game_session.slots.length; ++i) {
            var curr_slot_data = game_session.slots[i];
            var playerName = curr_slot_data? curr_slot_data.player.name: '';
            var imageSrc = (curr_slot_data && curr_slot_data.image_path) || '/static/images/no-drawing.png';
            var selected = event_data? event_data.selected.indexOf(i) > -1: false;
            summary_data.push({player: playerName, image: imageSrc, selected: selected});
            if (curr_slot_data && curr_slot_data.player === this.player) {
